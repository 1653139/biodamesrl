...
from biodamesrl.liir.dame.core.nn.WEDict import WEDict
...
class PredDataProcessor(DataProcessor):
    ...
    def process_sentence(self, sen):
        ...
        return [dt]
    def get_class_weights(self, train_path):
        positive, negative = 0,0
        for sen in CoNLL2005Reader(train_path).read_all():
            preds = len([w for w in sen if isinstance(w, Predicate)])
            positive+=preds
            negative+= len(sen)-preds
        return np.asarray([positive, negative])