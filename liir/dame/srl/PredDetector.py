...
from biodamesrl.liir.dame.core.io.CoNLL2005Reader import CoNLL2005Reader
from biodamesrl.liir.dame.core.nn.Data import BucketedDataIteratorSEQ, PaddedDataIteratorSEQ
from biodamesrl.liir.dame.core.nn.ExtHighWayWrapper import ExtHighWayLSTMCell, ExtDropoutWrapper
from biodamesrl.liir.dame.core.nn.thirdparties.Attention import attention_bias, multihead_attention, add_timing_signal
from biodamesrl.liir.dame.core.nn.thirdparties.Common import layer_norm, linear
from biodamesrl.liir.dame.core.representation.Predicate import Predicate
from biodamesrl.liir.dame.core.representation.Sentence import Sentence
from biodamesrl.liir.dame.core.representation.Text import Text
from biodamesrl.liir.dame.srl.CharWordEmbedding import CharProcessor
from biodamesrl.liir.dame.srl.ConfigReader import Configuration
from biodamesrl.liir.dame.srl.DataProcessor import DataProcessor, PredDataProcessor
...
class PredDetector:
    def __init__(self, config_path="config/pred.ini", tag_id=None):
        '''
        init a deep semantic role labeller
        :param config_path: path to configuaration file
        '''
        #self.class_weights = self.pp.get_class_weights(self.cfg.train_path)
        ...
        self.we_dict = None
        self.init()
        tp = [1,10]#self.pp.get_class_weights(self.cfg.train_path).tolist()
        self.class_weights = [float(tp[1]/tp[0]), 1.0]
    ...
    #this function is only used for att
    ...
    def embeddings(self):
        ...
        if "char" in self.cfg.emb_type:
            ...
            return tf.concat([word_embeddings, output], -1)
    ...
    def logits_for_rnn(self, input=None):
        ...
        all_inputs = [input]
        if self.cfg.multiply_embedding_mode == "sqrt_depth":
            input = input * (self.cfg.hidden_dim ** 0.5)
    ...
    @lazy_property
    def prediction(self):
        return tf.cast(tf.argmax( self.logits * tf.convert_to_tensor(np.asarray(self.class_weights, dtype="float32")), axis=-1),
                       tf.int32)
    ...
    def loss(self):
        ...
        if self.cfg.label_smoothing != 0.0:
            ...
            #losses = tf.nn.weighted_cross_entropy_with_logits(self.targets, self.logits, pos_weight=2.0)
    ...
    def init(self):
        ...
        self.pp = PredDataProcessor(model_dir=self.cfg.model_dir)
    ...
    def restore(self):
        ...
        def optimistic_restore_vars(model_checkpoint_path):
            ...
            print (saved_shapes)
            print ([var.name for var in tf.global_variables(scope="pred")])
            print ([var.name.split(':')[0] for var in tf.global_variables(scope="pred") if var.name.startswith("pred/")])

            var_names = sorted([(var.name, var.name.split(':')[0][5:]) for var in tf.global_variables()
                                if var.name.split(':')[0][5:] in saved_shapes and var.name.startswith("pred/")])
            print (var_names)

            restore_vars = {}
            name2var = dict(zip(map(lambda x: x.name, tf.global_variables(scope="pred")), tf.global_variables(scope="pred")))
            with tf.variable_scope('', reuse=True):
                for var_name, saved_var_name in var_names:
                    curr_var = name2var[var_name]
                    var_shape = curr_var.get_shape().as_list()
                    if var_shape == saved_shapes[saved_var_name]:
                        restore_vars[curr_var] = saved_var_name
            print (restore_vars)
            return restore_vars
    ...
    def get_feed(self, inputs_vals,  target_vals, seq_len_vals, dr=1.0, char_vals=None, char_lens = None, is_train=True, lr=None):
        if lr is None:
    ...
    def run_epoch(self, train_input, dev_input, brown_input, wsj_input, epc=None):
        ...
        while train_input.epochs == current_epoch:
            ...
            (inputs_vals, target_vals), seq_len_vals = train_input.next_batch(self.cfg.batch_size)
            ...
            feeddct = self.get_feed(inputs_vals, target_vals, seq_len_vals,
    ...
    def conll_evaluate(self, test_input, test, test_path):
        ...
        while test_input.epochs == current_epoch:
            (inputs_vals,  target_vals), seq_len_vals = test_input.next_batch(self.cfg.batch_dev_size)

            char_vals, char_lens = None, None
            if "char" in self.cfg.emb_type:
                char_vals, char_lens = self.charpp.get_data(inputs_vals)

            feeddct_err = self.get_feed(inputs_vals, target_vals, seq_len_vals,
        ...
        for sen in test:
            total += len(sen)
            gold += len(sen.get_predicates())

        for sen in test:

                temps = all_predictions[test.index(sen)][0:len(sen)]
                #print(temps)
                sys += len([x for x in temps if x == "True"])
                for x in temps:
                    if x == "True" and isinstance(sen[temps.index(x)], Predicate):
                        correct += 1
                    #if x=="False" and not isinstance(sen[temps.index(x)], Predicate):
                    #    correct+=1

                #correct+= len([x for x in temps if x=="True" and isinstance(sen[temps.index(x)], Predicate)])

        if sys == 0:
            P,R,F = 0, 0, 0
        else:
            P = float(correct/sys)
            R = float(correct/gold)
            if P + R != 0:
                F = float(2*P*R / (P+R))
            else:
                F=0

        return {"conll": F, "P": P, "R":R }

    def predict(self, test):

        test_data = self.pp.get_data_eval(None,
                                          test)
...
def main(config_path="config/srl.ini"):
    ...
    if dsrl.cfg.mode == "infer":
        ...
        dsrl.cfg.inference_type="argmax"
        for p in [dsrl.cfg.dev_path, dsrl.cfg.test_brown_path, dsrl.cfg.test_wsj_path]:
            print (dsrl.evaluate(p))

        dsrl.cfg.inference_type = "A*"
        for p in [dsrl.cfg.dev_path, dsrl.cfg.test_brown_path, dsrl.cfg.test_wsj_path]:
            print(dsrl.evaluate(p))
